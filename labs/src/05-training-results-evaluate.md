# Lab: Training Results Evaluate

## 📘 Lab Scenario
In this lab, you will work through the notebook src\05-training-results-evaluate.ipynb and execute each code cell in sequence to complete the workflow successfully.
...
my_client = project_client.get_openai_client(api_key=os.environ["AZURE_OPENAI_API_KEY"])
...
def run_agent(user_message, client, model="gpt-5-mini", verbose=True):
...
checkpoints = {
    "Base o4-mini": {"avg": 0.73, "p90": 0.37, "p80": 0.6},
    "Step 95 (peak train reward)": {"avg": 0.85, "p90": 0.46, "p80": 0.83},
    "Step 115 (best eval)": {"avg": 0.87, "p90": 0.5, "p80": 0.9},
}
