        ...
        if tc["name"] == "calculate_resolution" and tc.get("result"):
            try:
                ...
                for item in res.get("breakdown", []):
                    iid = item.get("item_id", "")
                    act = item.get("action", "")
                    if iid and act and iid not in actions:
                        actions[iid] = act
...
def score_tool_usage(result: dict, scenario: dict) -> float:
    ...
    for tc in result.get("tool_calls", []):
        ...
        if "order_id" in args and order_id:
            checks += 1
    ...
    for tc in tool_calls:
        if tc["name"] == "calculate_resolution" and tc.get("result"):
            try:
                ...
                actual["total_resolution"] = summary.get("total_resolution_amount")
                actual["total_restocking"] = summary.get("total_restocking_fees")
                actual["shipping_credit"] = summary.get("total_shipping_credit")
                actual["total_refund"] = summary.get("total_refund")
...
def evaluate_model(model: str, scenarios: list, client=None, verbose=False) -> dict:
    """Run all scenarios against a model and return aggregated results."""
    from .zava_agent import run_agent, get_client
