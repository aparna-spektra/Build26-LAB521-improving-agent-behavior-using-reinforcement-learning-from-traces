"""Upload v7 data and submit the baseline RFT job."""
...
try:
    ...
    print(f"  ❌ Error: {e}")
    job_entry = {"suffix": SUFFIX, "error": str(e)}
...
if "job_id" in job_entry:
    print(f"  ✅ Job submitted: {job_entry['job_id']}")
else:
    print(f"  ❌ Job failed: {job_entry['error']}")
